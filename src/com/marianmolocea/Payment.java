...
public class Payment {
    ...
    public static void makePayment(Snack item, byte quantity) {
        ...
        while(amountPayed < amountToBePayed) {
            System.out.println("Total left to pay: " + "\u001B[36m£" + (Math.round((amountToBePayed - amountPayed) * 100D) / 100D) + "\u001B[0m");
        ...
        if(amountPayed > amountToBePayed && (amountToBePayed - amountPayed) < 0.05D)
            System.out.println(
                    "Please collect your \u001B[33m" + item.name.trim() + "\u001B[0m and your change: \u001B[33m£"
                            + (Math.round((amountPayed - amountToBePayed) * 100D) / 100D) + "\u001B[0m"
            );
        else
